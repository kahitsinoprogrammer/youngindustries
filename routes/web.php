...
use App\Http\Controllers\Account\AssetChatbotController;
use App\Http\Controllers\ActionlogController;
use App\Http\Controllers\Api\ImportController;
use App\Http\Controllers\Account\AssetRequestController;
use App\Http\Controllers\Account\TechSupportController;
...
Route::group(['middleware' => ['auth']], function () {
    Route::get('assetRequest', [AssetRequestController::class, 'index'])
        ->name('asset-request.index')
        ->breadcrumbs(fn (Trail $trail) =>
        $trail->parent('home')
            ->push(trans('general.asset_request_menu'), route('asset-request.index')));

    Route::post('assetRequest', [AssetRequestController::class, 'store'])
        ->name('asset-request.store');

    Route::get('assetRequest/chatbot', [AssetChatbotController::class, 'index'])
        ->name('asset-request.chatbot')
        ->breadcrumbs(fn (Trail $trail) =>
        $trail->parent('home')
            ->push(trans('general.asset_request_menu'), route('asset-request.index'))
            ->push(trans('general.asset_chatbot'), route('asset-request.chatbot')));

    Route::post('assetRequest/chatbot/ask', [AssetChatbotController::class, 'ask'])
        ->name('asset-request.chatbot.ask');

    Route::get('assetRequest/status', [AssetRequestController::class, 'requestStatus'])
        ->name('asset-request.status')
        ->breadcrumbs(fn (Trail $trail) =>
        $trail->parent('home')
            ->push(trans('general.asset_request_menu'), route('asset-request.index'))
            ->push(trans('general.asset_request_status'), route('asset-request.status')));

    Route::get('assetRequest/approve', [AssetRequestController::class, 'approve'])
        ->name('asset-request.approve')
        ->breadcrumbs(fn (Trail $trail) =>
        $trail->parent('home')
            ->push(trans('general.asset_request_menu'), route('asset-request.index'))
            ->push(trans('general.asset_approve'), route('asset-request.approve')));

    Route::post('assetRequest/approve/{requestId}', [AssetRequestController::class, 'reviewApproval'])
        ->name('asset-request.approve.review');

    Route::get('assetRequest/asset-specification', [AssetRequestController::class, 'assetSpecification'])
        ->middleware('can:asset-request.asset-specification')
        ->name('asset-request.asset-specification')
        ->breadcrumbs(fn (Trail $trail) =>
        $trail->parent('home')
            ->push(trans('general.asset_request_menu'), route('asset-request.index'))
            ->push(trans('general.asset_specification'), route('asset-request.asset-specification')));

    Route::get('assetRequest/asset-specification/{requestId}', [AssetRequestController::class, 'showAssetSpecification'])
        ->middleware('can:asset-request.asset-specification')
        ->name('asset-request.asset-specification.show')
        ->breadcrumbs(fn (Trail $trail, string $requestId) =>
        $trail->parent('asset-request.asset-specification')
            ->push(trans('general.details'), route('asset-request.asset-specification.show', $requestId)));

    Route::post('assetRequest/asset-specification/{requestId}', [AssetRequestController::class, 'storeAssetSpecification'])
        ->middleware('can:asset-request.asset-specification')
        ->name('asset-request.asset-specification.store');

    Route::post('assetRequest/asset-specification/{requestId}/deploy', [AssetRequestController::class, 'markAssetSpecificationAsDeployed'])
        ->middleware('can:asset-request.asset-specification')
        ->name('asset-request.asset-specification.deploy');

    Route::get('assetRequest/assets-approved', [AssetRequestController::class, 'assetsApproved'])
        ->middleware('can:asset-request.asset-specification')
        ->name('asset-request.assets-approved')
        ->breadcrumbs(fn (Trail $trail) =>
        $trail->parent('home')
            ->push(trans('general.asset_request_menu'), route('asset-request.index'))
            ->push(trans('general.asset_specification'), route('asset-request.assets-approved')));

    Route::get('assetRequest/assignaApprover', [AssetRequestController::class, 'assignApprover'])
        ->middleware('can:superuser')
        ->name('asset-request.assign-approver')
        ->breadcrumbs(fn (Trail $trail) =>
        $trail->parent('home')
            ->push(trans('general.asset_request_menu'), route('asset-request.index'))
            ->push(trans('general.assign_approver'), route('asset-request.assign-approver')));

    Route::post('assetRequest/assignaApprover', [AssetRequestController::class, 'storeApproverAssignment'])
        ->middleware('can:superuser')
        ->name('asset-request.assign-approver.store');

    Route::get('assetRequest/assignTechnicalSupport', [AssetRequestController::class, 'assignTechnicalSupport'])
        ->middleware('can:superuser')
        ->name('asset-request.assign-technical-support')
        ->breadcrumbs(fn (Trail $trail) =>
        $trail->parent('home')
            ->push(trans('general.asset_request_menu'), route('asset-request.index'))
            ->push(trans('general.assign_technical_support'), route('asset-request.assign-technical-support')));

    Route::post('assetRequest/assignTechnicalSupport', [AssetRequestController::class, 'storeTechnicalSupportAssignment'])
        ->middleware('can:superuser')
        ->name('asset-request.assign-technical-support.store');

});

Route::group(['middleware' => ['auth']], function () {
    Route::get('tech-support/request-ticket', [TechSupportController::class, 'requestTicket'])
        ->name('tech-support.request-ticket')
        ->breadcrumbs(fn (Trail $trail) =>
        $trail->parent('home')
            ->push(trans('general.tech_support_menu'), route('tech-support.request-ticket'))
            ->push(trans('general.tech_support_request_ticket'), route('tech-support.request-ticket')));

    Route::post('tech-support/request-ticket', [TechSupportController::class, 'storeRequestTicket'])
        ->name('tech-support.request-ticket.store');

    Route::get('tech-support/ticket-status', [TechSupportController::class, 'ticketStatus'])
        ->name('tech-support.ticket-status')
        ->breadcrumbs(fn (Trail $trail) =>
        $trail->parent('home')
            ->push(trans('general.tech_support_menu'), route('tech-support.request-ticket'))
            ->push(trans('general.tech_support_ticket_status'), route('tech-support.ticket-status')));

    Route::get('tech-support/approve-ticket', [TechSupportController::class, 'approveTicket'])
        ->middleware('can:tech-support.approve-ticket')
        ->name('tech-support.approve-ticket')
        ->breadcrumbs(fn (Trail $trail) =>
        $trail->parent('home')
            ->push(trans('general.tech_support_menu'), route('tech-support.request-ticket'))
            ->push(trans('general.tech_support_approve_ticket'), route('tech-support.approve-ticket')));

    Route::post('tech-support/approve-ticket/{ticketId}', [TechSupportController::class, 'reviewTicket'])
        ->middleware('can:tech-support.approve-ticket')
        ->name('tech-support.approve-ticket.review');

    Route::get('tech-support/jira-diagnostics', [TechSupportController::class, 'jiraDiagnostics'])
        ->middleware('can:tech-support.jira-diagnostics')
        ->name('tech-support.jira-diagnostics')
        ->breadcrumbs(fn (Trail $trail) =>
        $trail->parent('home')
            ->push(trans('general.tech_support_menu'), route('tech-support.request-ticket'))
            ->push(trans('general.tech_support_jira_diagnostics'), route('tech-support.jira-diagnostics')));

    Route::get('tech-support/resolve-ticket', [TechSupportController::class, 'resolveTicket'])
        ->middleware('can:tech-support.resolve-ticket')
        ->name('tech-support.resolve-ticket')
        ->breadcrumbs(fn (Trail $trail) =>
        $trail->parent('home')
            ->push(trans('general.tech_support_menu'), route('tech-support.request-ticket'))
            ->push(trans('general.tech_support_resolve_ticket'), route('tech-support.resolve-ticket')));

    Route::post('tech-support/resolve-ticket/{ticketId}', [TechSupportController::class, 'closeTicket'])
        ->middleware('can:tech-support.resolve-ticket')
        ->name('tech-support.resolve-ticket.close');
});
